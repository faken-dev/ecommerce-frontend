  ...
  const fetchData = async () => {
    ...
    try {
      const [invRes, prodRes] = await Promise.all([
        inventoryApi.listAllAdmin({ page: page - 1, size: 20 }),
        catalogApi.listAllAdmin(page - 1, 20)
      ])
      ...
      if (invRes.data?.success && prodRes.data?.success) {
        ...
        if (invRes.data.page) {
          ...
          setTotalElements(invRes.data.page.totalElements)
        }
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.pageTitle}>Vận hành Kho & Tồn kho</h1>
          <p className={styles.pageSub}>Quản trị toàn bộ hàng hóa và vị trí lưu trữ trong hệ thống</p>
        </div>
      </div>
      ...
      <div className={styles.filters}>
        <div className={styles.searchInputWrapper}>
          ...
          <input 
            ...
            placeholder="Tìm kiếm theo tên sản phẩm, SKU hoặc danh mục..."
            value={searchTerm}
      ...
      <div className={styles.card}>
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className={styles.loading}>Đang xử lý dữ liệu...</td></tr>
              ) : filteredItems.length === 0 ? (
                <tr><td colSpan={6} className={styles.loading}>Hệ thống không có dữ liệu phù hợp.</td></tr>
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <Badge type={item.availableQuantity > 0 ? 'success' : 'error'}>
                      {item.availableQuantity} sp sẵn có
                    </Badge>
                  </td>
                  <td>
                    <div className={styles.adjustGroup}>
                      <button 
                        ...
                        onClick={(e) => {
                          ...
                          handleAdjust(item.productId, item.variantId, -Math.abs(val || 1))
                        }}
                      >-</button>
                      <input 
                        type="number" 
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(8px)' }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', padding: 32, borderRadius: 24, width: '100%', maxWidth: 700, maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
              <h2 style={{ margin: 0 }}>Nhật ký Biến động</h2>
              <button className={styles.actionBtn} onClick={() => setShowHistory(null)}><Icon.X size={20} /></button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {loadingLogs ? <div className={styles.loading}>Đang tải dữ liệu...</div> : logs.length === 0 ? <p style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>Không có bản ghi lịch sử.</p> : (
      ...
      {showSetLocation && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(8px)' }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', padding: 32, borderRadius: 24, width: '100%', maxWidth: 600 }}>
            <h2 style={{ marginBottom: 24 }}>Thiết lập vị trí lưu trữ</h2>
            
            <div style={{ marginBottom: 20 }}>
              <label className={styles.label}>Hệ thống Kho</label>
              <select 
            ...
            {selectedWarehouse && (
              <div style={{ maxHeight: 300, overflowY: 'auto', marginBottom: 24 }}>
                <label className={styles.label}>Vị trí kệ (Slot)</label>
                {structure.length === 0 ? <p className={styles.pageSub}>Kho này chưa được thiết lập sơ đồ.</p> : structure.map(zone => (
